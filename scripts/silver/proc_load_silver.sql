...
Parameters:
    None.
    This stored procedure does not contain any parameters or return any values.
	...
	BEGIN TRY
		...
		SELECT
			...
			CASE 
				...
				ELSE 'n/a'
			END AS cst_gndr,  --Normalise marital status values to readable format
			cst_create_date
		FROM(
			SELECT 
				*,
		...
		SELECT
			...
			prd_nm,
			COALESCE(prd_cost,0) AS prd_cost, --Handling NULL
